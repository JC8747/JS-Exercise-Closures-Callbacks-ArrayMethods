...
function processSum(numberList, callback) {
  const totalList = numberList.reduce((a,b) => a + b, 0);
  return callback(totalList);
}
...
function firstNamesAllCaps(runners) {
  const runnerNames = runners.map (runner => {
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  runners = shirt_size.filter(tShirtSize)
...
function tallyUpDonations(runners) {
  const donotally = runners.reduce((total, donation) => total + donation,0);
  return donotally
}
